...
def test_url(client, test_app):
    with mock.patch('psycopg2.connect', autospec=True) as mock_connect:
        ...
        mock_cur.fetchall.return_value = [('1', '1', 'None', 'None', 'None', 'None', '26-01-2023'),
                                          ('2', '1', 'None', 'None', 'None', 'None', '26-01-2023')]
        response = client.get('/urls/1', follow_redirects=True)
        assert response.status_code == 200
        mock_connect.assert_called_with(test_app.config['DATABASE_URL'])
        mock_cur.execute.assert_any_call("SELECT id, name, TO_CHAR(created_at, 'DD-MM-YYYY') "
        ...
        assert created_at1.encode("utf-8", "ignore") in response.data
        assert created_at2.encode("utf-8", "ignore") in response.data
